...
class AttributionHypothesis(BaseEntity):
    ...
    hypothesis_type: str = Field(
        ...,
        description="Type of attribution hypothesis"
    )
    subject: str = Field(..., description="What is being attributed (alias, post, etc.)")
    ...
    contradicting_evidence: List[UUID] = Field(
        default_factory=list,
        description="Evidence IDs contradicting this hypothesis"
    )
    explanation: Optional[str] = Field(
        default=None,
        description="Explanation of the hypothesis and evidence"
    )
    investigation: Optional[EntityReference] = Field(
        default=None,
        description="Investigation this hypothesis belongs to"
    )
