...
class Post(BaseEntity, ProvenanceMixin):
    ...
    author: Optional[EntityReference] = Field(
        default=None, description="Alias that authored this post"
    )
    mentioned_entities: List[EntityReference] = Field(
        default_factory=list, description="Entities mentioned in this post"
    )
    investigation: Optional[EntityReference] = Field(
        default=None, description="Investigation this post belongs to"
    )
