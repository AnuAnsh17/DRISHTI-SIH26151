...
class PGPKey(BaseEntity, ProvenanceMixin):
    ...
    first_seen: Optional[datetime] = Field(
        default=None, description="First time this PGP key was observed"
    )
    last_seen: Optional[datetime] = Field(
        default=None, description="Last time this PGP key was observed"
    )
    # Relationships
    actor: Optional[EntityReference] = Field(
        default=None, description="Actor that uses this PGP key"
    )
    posts: List[EntityReference] = Field(
        default_factory=list, description="Posts signed with this key"
    )
