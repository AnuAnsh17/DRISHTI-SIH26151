...
class AttributionStatus(str, Enum):
    """Attribution status for threat actors."""
...
class Actor(BaseEntity, ProvenanceMixin):
    ...
    first_seen: Optional[datetime] = Field(
        default=None, description="Earliest timestamp associated with this actor"
    )
    last_seen: Optional[datetime] = Field(
        default=None, description="Most recent timestamp associated with this actor"
    )
    canonical_name: Optional[str] = Field(
        default=None, description="Primary name or identifier (if known)"
    )
    ...
    aliases: List[EntityReference] = Field(
        default_factory=list, description="Aliases used by this actor"
    )
    posts: List[EntityReference] = Field(
        default_factory=list, description="Posts authored by this actor"
    )
    pgp_keys: List[EntityReference] = Field(
        default_factory=list, description="PGP keys used by this actor"
    )
    wallets: List[EntityReference] = Field(
        default_factory=list, description="Cryptocurrency wallets used by this actor"
    )
    onion_services: List[EntityReference] = Field(
        default_factory=list, description="Onion services linked to this actor"
    )
    infrastructure: List[EntityReference] = Field(
        default_factory=list, description="Infrastructure used by this actor"
    )
    behaviour_profile: Optional[EntityReference] = Field(
        default=None, description="Behaviour profile of this actor"
    )
    stylometric_profile: Optional[EntityReference] = Field(
        default=None, description="Stylometric profile of this actor"
    )
    trust_relationships: List[EntityReference] = Field(
        default_factory=list, description="Trust relationships with other actors"
    )
    ...
    investigation: Optional[EntityReference] = Field(
        default=None, description="Investigation this actor belongs to"
    )
