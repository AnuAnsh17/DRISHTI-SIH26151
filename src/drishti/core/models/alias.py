...
class Alias(BaseEntity, ProvenanceMixin):
    ...
    first_seen: Optional[datetime] = Field(
        default=None,
        description="First time this alias was observed"
    )
    last_seen: Optional[datetime] = Field(
        default=None,
        description="Last time this alias was observed"
    )
    # Relationships
    actor: Optional[EntityReference] = Field(
        default=None,
        description="Actor that uses this alias"
    )
    posts: List[EntityReference] = Field(
        default_factory=list,
        description="Posts where this alias appears"
    )
