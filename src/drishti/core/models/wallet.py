...
class Wallet(BaseEntity, ProvenanceMixin):
    ...
    first_seen: Optional[datetime] = Field(
        default=None,
        description="First time this wallet was observed"
    )
    last_seen: Optional[datetime] = Field(
        default=None,
        description="Last time this wallet was observed"
    )
    # Relationships
    actor: Optional[EntityReference] = Field(
        default=None,
        description="Actor that owns this wallet"
    )
    transactions: List[EntityReference] = Field(
        default_factory=list,
        description="Transactions involving this wallet"
    )
