...
from src.drishti.core.models.actor import Actor
...
def test_actor_creation():
    ...
    assert actor.attribution_status == "pending"  # default
...
def test_actor_with_all_fields():
    ...
    assert actor.attribution_status == "confirmed"
...
def test_actor_attribution_status_validation():
    ...
    for status in ["pending", "confirmed", "rejected", "disputed"]:
        actor = Actor(
            source="test",
            source_type="test",
